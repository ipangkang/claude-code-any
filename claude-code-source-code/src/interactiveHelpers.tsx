...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  // anycode: check provider config, show setup if missing
  let _skipAnthropicSetup = false;
  try {
    const { hasProviderConfig: _hpc } = await import('./services/api/providerConfig.js');
    _skipAnthropicSetup = _hpc();
  } catch {}

  // anycode: show provider setup if no config exists
  if (!_skipAnthropicSetup) {
    const { ProviderSetup } = await import('./components/ProviderSetup.js');
    await showSetupDialog(root, done => <ProviderSetup onDone={() => void done()} />, {
      onChangeAppState
    });
    // Re-check after setup
    try {
      const { hasProviderConfig: _hpc2 } = await import('./services/api/providerConfig.js');
      _skipAnthropicSetup = _hpc2();
    } catch {}
  }

  let onboardingShown = false;
  if (!_skipAnthropicSetup && (!config.theme || !config.hasCompletedOnboarding)) {
  ...
  } else if (_skipAnthropicSetup && !config.hasCompletedOnboarding) {
    // anycode: auto-complete onboarding when using custom provider
    completeOnboarding();
  }
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    // anycode: auto-accept trust when custom provider is configured
    if (_skipAnthropicSetup) {
    ...
    // anycode: skip approval dialogs when using custom provider
    if (!_skipAnthropicSetup) {
      const {
        errors: allErrors
  ...
  // anycode: skip Anthropic-specific dialogs when using custom provider
  if (!_skipAnthropicSetup) {
